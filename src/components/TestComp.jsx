...
// 어떤 값이 들어올지 모를 때 ...rest를 통해서 한 번에 받아옴 > 객체형식으로 들어옴
const TestComp = ({ children, size, color, ...rest }) => {
  ...
  // onClick이벤트를 사용하기 위해서 {}를 통해 자바스크립트 형식으로 받아온다

  // rest 사용법이 헷갈리면 onClick을 속성값으로 가져와서 확인해 보기
  // props값: onClick,  사용법: onClick={onClick}으로 사용
  return (
    <button
      className={classNames("MyButton", size, color)}
      {...rest}
